...
const BigCategoryContainer: React.FC<CategoryProps> = ({ type }) => {
  ...
  useEffect(() => {
    bigCategoriesService
      .getCategoryByType(TypeEnum[type as keyof typeof TypeEnum])
      .then((res) => {
            ...
            {dataSource.length >= 1 ? (
              <div>
                <Popconfirm okType="default" title="DO YOU WANT TO DELETE?" onConfirm={() => handleDelete(record.id)}>
                  <Button type="default" className="hover:bg-[#777] hover:!text-[#fff] hover:!border-[#777]">
                    Delete
                  </Button>
                </Popconfirm>
              </div>
  ...
  const handleSave = async (row: DataType) => {
    ...
    if (row.id === -1) {
      const res = await bigCategoriesService.createBigCategory({
        name: row.name,
        typeId: TypeEnum[type as keyof typeof TypeEnum],
      })
      row.id = res.data.id
    } else {
  ...
  return (
    <div className="bg-[#FFFFFF] w-[100%] p-4">
      <div className="flex justify-between pb-5 items-center">
        <div className="text-[30px] text-baseColor">
          Type : <span className="font-medium text-[36px] uppercase">{type}</span>
        </div>
        <button
          className="w-[180px] h-[40px] bg-white border  hover:bg-baseColor hover:text-[#FFFFFF] rounded-md transition-all"
          onClick={handleAdd}
