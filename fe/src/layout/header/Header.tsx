  ...
  useEffect(() => {
    getTypeTree().then((res) => {
      ...
      const typeTree = typeArray.map((el: any) => {
        ...
        const menuArray = arrangeMenu(catArrayes, 4).map((element: any) =>
          element.map((element1: any) => el.big_categories[element1.id])
        )
    ...
    <>
      <div className="flex-col bg-[#ffffff] sticky top-0 z-[1000]">
        <div className="h-[40px]  flex items-center justify-end  border-b border-[#ebebeb] pl-[8%] pr-[8%] bg-[#F4F4F4]">
          {isLoggedIn ? (
            <p className="text-[#111111] text-[14px] hover:text-black whitespace-nowrap font-normal ">
              Hello, {user?.name}
            </p>
          ) : (
            <p className="text-[#111111] text-[14px] hover:text-black whitespace-nowrap  font-normal">
        ...
        <div className="flex justify-between items-center h-[100px]  pl-[8%] pr-[8%] border-b border-[#ebebeb]">
          <a href="/" className="text-[32px] basis-[20%]  whitespace-nowrap text-baseColor ">
            T - STORE
          </a>
          <div className="flex">
            {!isLoading &&
...
const menuCustom = (array: any, rootName: string): ItemType[] => {
  return array.map((element: any, key: number) => ({
    type: 'group',
    children: element.map((element1: any) => ({
      type: 'group',
      label: (
        <span className="!text-[#000000]">
          <p className="font-[500]">{element1.name?.toString().toUpperCase()}</p>
        </span>
      ),
      children: element1.categories.map((element2: any) => ({
        label: (
          <a className="" href={`/${rootName.toLowerCase()}?id=${element1.id}-${element2.id}`}>
            {element2.name}
          </a>
        ),
        key: `${element2.id}`,
